...
func (d *Driver) UpdateCells(ctx context.Context, tableName string, changes []model.CellChange, schema *model.TableSchema) (int, error) {
	...
	for _, ch := range changes {
		...
		args := []any{sqlValue(ch.NewValue)}
		whereParts := make([]string, len(pkCols))
		for i, pk := range pkCols {
			whereParts[i] = fmt.Sprintf("%s = $%d", quoteIdent(pk), i+2)
...
func isBlobColumn(schema *model.TableSchema, name string) bool {
	for _, c := range schema.Columns {
		if c.Name == name {
			...
			if c.NativeType != nil {
				upper = strings.ToUpper(*c.NativeType)
			}
			return strings.Contains(upper, "BYTEA") || strings.Contains(upper, "BLOB")
		}
	}
	return false
}
