...
func (d *Driver) profileColumn(ctx context.Context, fromClause string, col model.ColumnInfo) (model.ColumnProfile, error) {
	if !sqlutil.IsSafeQuotedIdentifier(col.Name) || strings.Contains(col.Name, "`") {
	...
	if isNumericOrDateType(col.DataType) {
		...
		q = fmt.Sprintf("SELECT MIN(%s), MAX(%s) FROM %s WHERE %s IS NOT NULL", colRef, colRef, fromClause, colRef)
...
func (d *Driver) profileTopValues(ctx context.Context, fromClause, colName string) ([]model.ValueCount, error) {
	...
	q := fmt.Sprintf(
		`SELECT CAST(%s AS CHAR), COUNT(*) AS cnt FROM %s WHERE %s IS NOT NULL GROUP BY %s ORDER BY cnt DESC LIMIT %d`,
...
func isNumericOrDateType(dataType string) bool {
	...
	keywords := []string{"INT", "DECIMAL", "NUMERIC", "FLOAT", "DOUBLE", "REAL", "DATE", "TIME", "YEAR"}
