...
func (d *Driver) UpdateCells(ctx context.Context, tableName string, changes []model.CellChange, schema *model.TableSchema) (int, error) {
	...
	for _, ch := range changes {
		if !sqlutil.IsSafeQuotedIdentifier(ch.ColumnName) || strings.Contains(ch.ColumnName, "`") || !colSet[ch.ColumnName] {
		...
		for i, pk := range pkCols {
			whereParts[i] = fmt.Sprintf("%s = ?", quoteIdent(pk))
			args = append(args, sqlValue(ch.PrimaryKey[pk]))
		}

		setClause := fmt.Sprintf("%s = ?", quoteIdent(ch.ColumnName))
		query := fmt.Sprintf("UPDATE %s SET %s WHERE %s", ref.FromRef, setClause, strings.Join(whereParts, " AND "))
