...
func (d *Driver) profileTopValues(ctx context.Context, fromClause, colName string) ([]model.ValueCount, error) {
	colRef := fmt.Sprintf("%q", colName)
	q := fmt.Sprintf(
		`SELECT CAST(%s AS TEXT), COUNT(*) AS cnt FROM %s WHERE %s IS NOT NULL GROUP BY %s ORDER BY cnt DESC LIMIT %d`,
...
func isNumericOrDateType(dataType string) bool {
	...
	keywords := []string{"INT", "REAL", "FLOAT", "DOUBLE", "NUMERIC", "DECIMAL", "DATE", "TIME"}
