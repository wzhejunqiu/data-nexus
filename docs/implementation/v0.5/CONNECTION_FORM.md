...
| 常规 | 文件路径 | path + 浏览 | —      | **是**（create 可留空，保存时弹出文件选择器） |                                     |
...
- [x] Dialog 浮动于主界面，非全屏子页
- [x] 三方言字段矩阵正确显示/隐藏
- [x] **必填字段标签带红色 `*`；SQLite 显示名称为「可选」**
- [x] **保存 / 测试连接：缺必填项时 inline 错误、不发起 API**
- [x] **编辑模式密码留空可保存；新建远程连接密码必填**
- [x] **校验失败时自动展开含错误字段的分区并滚动到位**
- [x] MySQL：charset、collation、TLS、默认存储引擎可保存并生效
- [x] PostgreSQL：sslMode 全量、clientEncoding 可保存
- [x] SQLite：只读/WAL 在高级区
- [x] 编辑连接复用同一表单 UI
- [x] 测试连接 + Vault 解锁流程正常
